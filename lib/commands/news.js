	...
	async _distribute(article)
	{
		...
		for (let subscriptionChannel of channels)
		{
			try
			{
				...
				// Wait for 5 mins before sending articles, may be a resolution to issue #17
				// https://github.com/Medallyon/the-tamriel-messenger/issues/17
				setTimeout(async () =>
				{
				...
				components: [
					{
						...
						"options": Array.from(articles.values()).sort((a, b) => b.date - a.date).map(x => ({
							label: x.title.length <= 100 ? x.title : `${x.title.trim().slice(0, 99) + "…"}`,
							description: x.description.length <= 100 ? x.description : `${x.description.trim().slice(0, 99) + "…"}`,
							value: x.id.toString()
						}))
					}
				]
