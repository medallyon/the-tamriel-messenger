...
function executeCommand(msg)
{
    ...
    for (const cmd in client.modules)
    {
        ...
        for (const alias of command.meta.alias)
        {
            ...
            client.modules[cmd](msg).then(function(value)
            {
                if (!value || value instanceof Discord.Message)
                    return;

                if (!(value instanceof Discord.RichEmbed))
                    value = client.utils.createCommandResponseEmbed("success", value, msg);

                msg.channel.send({ embed: value }).catch(console.error);
            }).catch(function(error)
            {
                ...
                error = client.utils.createCommandResponseEmbed("error", error, msg);
                msg.channel.send({ embed: error }).catch(console.error);
            });
        }
    }
}
