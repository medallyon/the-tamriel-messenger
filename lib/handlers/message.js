...
function fetchGuildConfig(guild)
{
    return new Promise(function(resolve, reject)
    {
        if (!guild)
            return reject(new TypeError("'guild' cannot be undefined"));

        if (guild.config && Date.now() - guild.config.lastChecked < 1000 * 30)
            return reject(new Error("Checked for a new guild config version too soon. Wait at least 30 seconds before trying again"));

        client.api.get(`/get/servers/${guild.id}`)
            .then(function(config)
            {
                guild.config = config;
                guild.config.lastChecked = Date.now();
...
function getCommandArgs(msg)
{
    ...
    if (splitMsg[0].startsWith(msg.guild ? (msg.guild.config ? msg.guild.config.prefix : client.Client.DEFAULT_PREFIX) : client.Client.DEFAULT_PREFIX))
    {
        msg.command = splitMsg[0].slice((msg.guild ? (msg.guild.config ? msg.guild.config.prefix : client.Client.DEFAULT_PREFIX) : client.Client.DEFAULT_PREFIX).length).toLowerCase();
...
function executeCommand(msg)
{
    ...
    for (const cmd in client.modules)
    {
        ...
        for (const alias of command.alias)
        {
            ...
            {
                if (typeof value !== "string" && typeof value !== "number" && !(value instanceof Discord.RichEmbed) && !(value instanceof Discord.MessageEmbed))
                    return;

                if (!(value instanceof Discord.RichEmbed) && !(value instanceof Discord.MessageEmbed))
