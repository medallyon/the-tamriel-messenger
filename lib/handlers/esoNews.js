...
function esoNews(article)
{
    ...
    const newsRole = esoiGuild.roles.get("320932080966172672")
        , originallyMentionable = newsRole.mentionable;

    console.log(newsRole.name, originallyMentionable);

    _toggleNewsRoleMentionable(newsRole, true)
        .catch(console.error)
        .finally(function()
        {
            const newsChannel = esoiGuild.channels.get("171044943744335872");
            console.log(newsChannel.name);
            if (!newsChannel)
