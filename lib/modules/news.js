...
function news(msg)
{
    ...
    {
        if (!articles.length)
            return msg.reply("There are currently no News Articles to display. Come back later!").catch(console.error);

        if (articles.length === 1)
            return msg.reply(createEmbedFromArticle(articles[0])).catch(console.error);
        ...
        {
            collectorMessage.edit(createEmbedFromArticle(articles[selectedNumber - 1])).catch(console.error);
        }).catch(console.error);
    });
}
