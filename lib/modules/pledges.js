...
class pledges extends Command
{
        ...
        return new Promise(function(resolve, reject)
        {
            ...
            fs.readJson(path.join(client.Client.__paths.persistence, "modules", "pledges", "metadata.json"))
                ...
                .finally(function()
                {
                    ...
                    {
                        if (msg && msg.args && msg.args.length && ["maj", "gli", "urg"].some(x => msg.args[0].toLowerCase().includes(x)))
    ...
    async createIndividualPledgeEmbed(meta, arg, pledges)
    {
        ...
        embed.setAuthor(`${pledgeGiver}'s Pledge`, "https://images.uesp.net/8/81/ON-icon-achievement-A_Crown_of_Your_Own_Trousers.png")
        ...
        embed.addField("\u200b", `**${pledgeGiver}'s upcoming pledges**\n` + (meta
    ...
    async createCurrentPledgesEmbed(meta, pledges)
    {
        let embed = new Embed().setAuthor("Today's Pledges", "https://images.uesp.net/8/81/ON-icon-achievement-A_Crown_of_Your_Own_Trousers.png", "https://en.uesp.net/wiki/Online:Undaunted#Pledges")
            .setDescription("What are Pledges? Find out more about [Pledges and the Undaunted](https://en.uesp.net/wiki/Online:Undaunted).");
